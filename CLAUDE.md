...
### 個人依存設定の禁止（最重要）
**このリポジトリ (`ansible-local`) は public であり、汎用的な playbook 管理のみを行う。個人を特定できる情報を絶対に含めないこと。**

禁止事項:
- 個人名・フルネーム・メールアドレス・ID などをファイルに書き込まない
- 個人の絶対パス（例: `/Users/<name>/src/...`）をコミット対象に含めない
- マシン固有・ユーザー固有の設定をこのリポジトリに書かない
- `host_vars/` や個人向け変数ファイルをこのリポジトリに追加しない
- コミットメッセージ・PRディスクリプション・Issueに、他の private リポジトリ／プロジェクトの詳細（プロジェクト名、連携先サービス名、内部API仕様等）を書かない

個人依存の設定は private リポジトリ `ansible-private` 側で管理する。このリポジトリはあくまで「誰でも再利用できる汎用 playbook」の置き場所。
