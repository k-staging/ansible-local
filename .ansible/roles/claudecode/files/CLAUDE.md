...
### Step 1: 計画立案
**PRテンプレートを検索して読み込む**（Globで `.github/pull_request_template*`, `.github/PULL_REQUEST_TEMPLATE*`, `pull_request_template*` を検索し、見つかったらReadで内容を読む）。PRディスクリプション（概要・変更内容）を**簡潔に**作成し、ユーザーに提示して承認を得てから実装に着手する。

### Step 2-4: 実装・テスト・レビュー
**Superpowers のスキルに従って進める。** Superpowers が提供する brainstorming、writing-plans、test-driven-development、systematic-debugging、verification-before-completion 等のスキルを活用すること。

加えて、以下のプロジェクト固有ルールを守る：
- 編集前に対応する自前 Skill を呼ぶ（後述「Skills - 編集前に呼ぶ」参照）
- 実装完了後にプロジェクト固有のサブエージェントでレビュー（後述「Agents - レビュー時に呼ぶ」参照）
- `regression-checker` サブエージェントでデグレッションチェック
