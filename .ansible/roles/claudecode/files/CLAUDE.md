...
### Step 2: 実装
1. **編集前に同ディレクトリの既存ファイルを最低2つ読み、既存のスタイル・パターンを把握する**
2. 必要に応じてSkillを呼び出しながらコードを修正（既存コードのスタイルを踏襲すること）
3. 修正完了後、変更ファイル一覧を確認
...
### Step 4: レビュー
> **⚠️ レビュー用サブエージェントは1つずつ実行すること（前のエージェントが完了してから次を起動する）**
...
- **Push前（必須）** → `workflow-guardian`（全工程の実施状況を監査）

## 6. Skill の自動確認を行わない
**セッション開始時やタスク着手前に、適用可能な Skill があるか逐一確認・アナウンスしない。**
- 明らかに必要な Skill はアナウンスなしで使う
- Skill の適用可否を毎回ユーザーに見せる必要はない

## 7. 指示には素直に従う - 勝手な代替案を出さない
**ユーザーが明確な指示を出したら、別解釈・別案を添えずに指示どおりの計画を提示する。**
- 「こうしたほうが良いのでは？」と思っても、まず指示どおりの案を第一候補として出す
- 代替案を出すのは、指示に明らかな矛盾・危険性があるとき、またはユーザーから意見を求められたときに限る
- 指示の言葉を literal に拡大解釈しない（例: 「最新バージョンにして」を `latest` タグ運用と読み替える等、運用方針にまで踏み込んだ代替提案はしない）
...
- **実行環境・実行基盤を先に特定してから調査する**: ログ・メトリクスを見る前に「どの環境（staging/production 等）」「どの実行基盤（コンテナ／サーバ／キュー基盤の種別）」でそのジョブ・リクエストが動いているかを確定する。環境を取り違えたまま診断しない
- **マイグレーション・デコミッション状況の確認は git history を一次情報とする**: 「現在の稼働状態」ではなく、`git log` / PR 履歴から「何が変更されたか・何が廃止されたか」を読む。現在の状態から過去の意図を推測しない
- **既存システムの状態を verify せずに断定しない**: 「X は既に有効」「Y は既に廃止済み」等の状態は、設定ファイル・コマンド出力で確認してから述べる。記憶・推測で断定しない
- **ユーザーが提示したスクリーンショット・ログ・成果物は、仮説を立てる前に必ず読む**

### クラウド操作のコスト意識
クラウドリソースに対する高コストな操作は、実行前にコストを見積もり、必要に応じて確認を取る：

- **BigQuery**: クエリ実行前に `--dry-run` か `EXPLAIN` でスキャンサイズを見積もる。数 GB 以上をスキャンする見込みの場合はユーザー確認を取ってから実行する。`--maximum_bytes_billed` を活用する
- **オブジェクトストレージ（S3 / GCS 等）**: バケット全体の列挙・コピー・削除は事前にサイズ・オブジェクト数を見積もる。`--dryrun` 系フラグがある場合は先に実行する
- **ログ基盤**: 広範囲の横断検索は、まずサンプリング or 短期間・特定リソースに絞る。BigQuery 経由のログ全文検索は最終手段として扱う
- **コンテナ実行基盤（Cloud Run Jobs 等）でのジョブ実行**: 実行前にイメージタグ・リビジョン・投入パラメータが意図通りか確認する
- **長時間・高コストが想定される処理は、実行前に想定コスト・実行時間・対象範囲を明示してから着手する**

### PR Hygiene
PRを単一目的・最小 diff に保つための規律：

- **フォーマッタによる自動修正（例: rubocop、prettier 等）で意図しない差分が混入した場合は、一旦 reset し、意図した編集のみを再適用する**。整形差分と本質的な変更を同一 PR に混ぜない
- **自分（Claude）が作成した PR を自分で approve しない**。レビューは必ずコメント形式で投稿する（`gh pr review --comment`）
- **レビューコメントは「実在するリスク」のみ指摘する**。フレームワークの既定挙動や一般論をリスクとして書かない、裏付けのない speculative な指摘はしない
- **PR に関係ないリファクタリング・整理提案をレビューで混ぜない**
- 複数論点を混ぜず、1 PR 1 目的を徹底する
